	...
	*/
	...
	function Viewer(a) {
		...
		document.addEventListener("mousedown", (event) => {
			if (isControlDown) { //(event.ctrlKey) {
				...
				else if (isSelecting == 2) {
					// const selectionRectangle = document.querySelector(".selection-rectangle");
					isSelecting = 0;
					const width = event.clientX - initialX;
					const height = event.clientY - initialY;

					selectionRectangle.style.width = `${Math.abs(width)}px`;
					selectionRectangle.style.height = `${Math.abs(height)}px`;

					selectionRectangle.style.left = `${width > 0 ? initialX : event.clientX}px`;
					selectionRectangle.style.top = `${height > 0 ? initialY : event.clientY}px`;


					items.forEach((item) => {
						if (isElementInsideRect(item, selectionRectangle)) {
							item.style.backgroundColor = "#3d9f69b3"; // Hintergrundfarbe setzen
							item.style.color = "white"; // Textfarbe setzen
							selectedItems.push(item);
						}
						else {
							item.style.backgroundColor = "#5d5dd533";
		...
		function isElementInsideRect(element, rect) {
			const elementRect = element.getBoundingClientRect();
			const rectRect = rect.getBoundingClientRect();

			return (
				elementRect.left < rectRect.right &&
				elementRect.right > rectRect.left &&
				elementRect.top < rectRect.bottom &&
				elementRect.bottom > rectRect.top
			);
		}
