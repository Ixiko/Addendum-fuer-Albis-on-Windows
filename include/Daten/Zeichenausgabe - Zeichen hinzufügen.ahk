...
Gui DBL: new, +hwndhDBL
Gui DBL: +Lastfound

Gui Add, Text, xm ym , Zeichen die hinzugefügt werden sollen
Options := " xm y+5 w1200 r10 gREMessageHandler"
If !IsObject(RE := New RichEdit("DBL", Options)) {
   MsgBox, 16, Error, %ErrorLevel%
   ExitApp
}
RE.SetBorder([10], [2])
RE.SetBkgndColor(0x555555)
RE.SetFont({"Name":"Segoe UI Emoji","Color":0xFFFFFF,"Size":12})
RE.HideSelection(true)
RE.ID := DllCall("GetWindowLong", "UPtr", RE.hWnd, "Int", -12) ; GWL_ID
