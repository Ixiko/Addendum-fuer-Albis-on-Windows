...
admObjekte() {                                                                                       	;-- Addendum-Objekt erweitern
		...
		Addendum.Dicom                  	:= Object()       	; Einstellung für Dicom Dateiordner
		...
		Addendum.PDF.Faxbox         	:= Object()         	; Faxmanager Einstellungen
		Addendum.Praxis                     	:= Object()        	; Praxisdaten
		...
		Addendum.TProtocol               	:= Array()		    	; alternatives Tagesprotokoll
...
admVerzeichnisse() {                                                                             	;-- Programm- und Datenverzeichnisse (!nach admObjekte aufrufen!)
	...
	; Ordnerstruktur anlegen für log files
		If (!InStr(Addendum.LogPath, "Error") && Addendum.LogPath && isFullFilePath(Addendum.LogPath)) {

			If !InStr(FileExist(Addendum.LogPath), "D")
				FileCreateDir, % Addendum.LogPath

			If !InStr(FileExist(Addendum.LogPath "\ErrorLogs"), "D")
				FileCreateDir, % Addendum.LogPath "\ErrorLogs"

		}

	; Ordnerstruktur anlegen für die Verwaltung eingegangener Befunde, wenn ein BefundOrder in der ini angegeben wurde
		If (!InStr(Addendum.BefundOrdner, "Error") && Addendum.BefundOrdner && isFullFilePath(Addendum.BefundOrdner)) {
...
ChronikerListe() {                                                                                  	;-- Chroniker Leistungskomplexe 03220/03221
	...
	If FileExist(filepath := Addendum.DBPath "\PatData\DB_Chroniker.txt") {
		For idx, line in StrSplit(FileOpen(filepath, "r", "UTF-8").Read(), "`n", "`r")
			If RegExMatch(line, "^\d+", PatID)
				Addendum.Chroniker.Push(PatID)
	} else
		SciTEOutput("Chronikerliste nicht gefunden")
...
GeriatrischListe(LoadPatients:=true) {                                                     	;-- ICD Ziffern der Geriatrie und Patienten mit abgerechnetem GB
	...
	If LoadPatients
		If FileExist(filepath := Addendum.DbPath "\PatData\DB_Geriatrisch.txt") {
			For idx, line in StrSplit(FileOpen(filepath, "r", "UTF-8").Read(), "`n", "`r")
				If RegExMatch(line, "^\d+", PatID)
					Addendum.geriatrisch.Push(PatID)
		} else
			SciTEOutput("Liste mit Pat. für die Abrechnung des geriatrischen Basiskomplexes nicht gefunden")
	...
	; DicomCD_AutoCopy
		If DriveTypeExist("CDROM") {
			...
			If (!Addendum.Dicom.Dir || InStr(Addendum.Dicom.Dir, "ERROR")) {
				...
				Addendum.Dicom.Dir :=  Addendum.Dicom.Dir="ERROR" ? "" : Addendum.Dicom.Dir

			 ; AutoCopy überhaupt gewollt
				Addendum.Dicom.AutoCopy := false
				MsgBox, 0x1004, % StrReplace(A_ScriptName, ".ahk")	, % 	"An diesem PC ist ein angeschlossenes CD-Laufwerk gefunden worden.`n"
						                                                                    	.		"Möchten Sie auf diesem PC automatusch Kopien von DICOM-CDs`n"
						                                                                    	.   	"erstellen lassen. Dann drücken Sie auf Ja bitte."
				IfMsgBox, Yes
					Addendum.Dicom.AutoCopy := true
				IniWrite, % (Addendum.Dicom.AutoCopy ? "ja" : "nein"), % Addendum.ini, % compname, % "DicomCD_AutoCopy"
				If !Addendum.Dicom.AutoCopy
					return
				MsgBox, 0x1001, % StrReplace(A_ScriptName, ".ahk")	, % 	"Sie wollen automatisiert Kopien von DICOM-CD's erstellen lassen.`n"
																								, 		"Als nächstes wird ein Basisordner auf einer Festplatte benötigt,`n"
																								,		"in welches die CD-Daten getrennt nach untersuchtem Patient`n
																								,		"abgelegt werden sollen.`m"
																								,		"Drücken Sie auf OK wenn es weiter gehen soll."
...
admLanProperties() {                                                                              	;-- Netzwerkeinstellungen / Client PC Namen / IP-Adressen
	...
	; prüft auf korrekt gesetzten Computernamen
	...
	; Remotedesktop-Dateipfad, falls verfügbar
		If !InStr(FileExist(rdpPath := IniReadExt(compname, "rdpPath")), "D")
			rdpPath := ""
...
admFaxbox() {                                                                                         	;-- Faxbox wie Mailbox
	...
	; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		...
		Addendum.PDF.Faxbox.TrayTip             	:= IniReadExt("ScanPool", "FritzFaxbox_FaxEingangMelden", 4)
	  ...
	  ; nicht verwendbare Werte korrigieren
		...
		If !RegExMatch(Addendum.PDF.Faxbox.Interval, "^\d+$") {
			Addendum.PDF.Faxbox.Interval := 60
			IniWrite, 60, % Addendum.Ini, % "ScanPool", % "FritzFaxbox_Abrufintervall"
		}
		If !RegExMatch(Addendum.PDF.Faxbox.FaxEingangMelden, "^\d+$") {
			Addendum.PDF.Faxbox.TrayTip := 4
			IniWrite, 4, % Addendum.Ini, % "ScanPool", % "FritzFaxbox_FaxEingangMelden"
		}
		If !RegExMatch(Addendum.PDF.Faxbox.Preview, "^\d+$") {
			Addendum.PDF.Faxbox.Preview := 3
			IniWrite, 3, % Addendum.Ini, % "ScanPool", % "FritzFaxbox_Vorschau"
		}

	 ; lädt hier die Daten für den Faxfilter, wenn dies der PC ist, welcher die Faxboxfunktion ausführen soll
	   If (Addendum.PDF.Faxbox.Client = compname)
			admFaxboxFilterLoad()
		...
		If FileExist(path := IniReadExt("ScanPool", "FritzFaxbox_Telefonbuch")) {
			...
			For each, line in StrSplit(temp, "`n", "`r")
				If (StrSplit(line, "|").1 ~= "i)\(.*Fax.*?\)") {
					...
					Addendum.PDF.Faxbox.Telefon[nr] := {"sdr":sdr}
					If ftr
						Addendum.PDF.Faxbox.Telefon[nr].ftr := ftr
				}
		}

	; Blacklist laden (Faxe werden gelöscht)
	  ...
	  ; legt eineTextdatei mit Hinweisen zur richtigen Befüllung an
		else {
			filecontent =
			(LTrim
...
admTagesProtokoll() {                                                                            	;-- Anlegen/Lesen des aktuellen Tagesprotokolls
		...
		Addendum.TPSectionDate 	:= A_DDDD "|" A_DD "." A_MM                                         					;z.B. [Montag|15.10.]
	...
	; Tagesprotokoll Daten des Clients
	; ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ ᚔ
		If !FileExist(Addendum.TPFullPath)
			...
			TProtocol := Addendum.TProtocol := TProtGetDay(A_YYYY A_MM A_DD, compname)
}
...
ReadIniSection(Section, defaultKeys:="") {

	; default keys als String übergeben. Format wie in einer ini-Datei

	lastencoding := A_FileEncoding
	FileEncoding, UTF-16
	IniRead, inikeys, % Addendum.Dir "\Addendum.ini", % Section
	FileEncoding, % lastencoding

	If (InStr(inikeys, "ERROR") || !inikeys)
		return

	inisection := Object()
	For inikey, inival in StrSplit(inikeys, "`n", "´r")
		inisection[inikey] := Trim(inival)
