...
;                        	by Ixiko started in September 2017 - last change 20.09.2022 - this file runs under Lexiko's GNU Licence
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
tessOCRPdf(PDFName, params)                                         	{              	;-- tesseract Texterkennung -> erstellt eine durchsuchbare PDF Datei
  ...
  ; Einstellungen
	...
	; Parameter parsen                    	;{
		...
		If !IsObject(tess) || (params.SetName <> lastSetName) {
			...
			PDFName  	:= RegExReplace(PDFName, "\.pdf\s*$")
  ...
  ; Texterkennung
	...
	; Seitenbilder extrahieren            	;{
		...
		; Originalbilder entpacken
			...
			If (params.debug > 1)
				SciTEOutput("  - pdfconvert (cmdline): " pdfconvert)
		 ...
		 ; entfernt den Textlayer aus einer durchsuchbaren PDF
			...
			; Bilder entpacken
				...
				tess.imgpattern := 0, tess.imgext := "jpg"
				pdfconvert	:= tess.imagick " -units PixelsPerInch -density 300 " q tess.tempPath "\in.pdf" q " " q tess.tempPath "\conv." tess.imgext q
				;~ pdftoimg	:= tess.imagick " -units PixelsPerInch -density 200 " q tess.tempPath "\in.pdf" q " " q tess.tempPath "\conv." tess.imgext q
	...
	; OCR preprocessing                 	;{
	  ...
	  ; Vorbereitung der Seitenbilder (nicht imagemagick gestützt)
		If (tess.preprocessing != "imagemagick")	{

			SciTEOutput(" - Preprocessing: eingestellt ist " tess.preprocessing ", es wird imagemagick verwendet")
			tess.preprocessing := "imagemagick"
	  ...
	  ; infile und config file erstellen     	;{
		...
		Loop, Files, % tess.tempPath "\*." workExt
			pages := A_Index, workimgs .= (A_Index > 1 ? "`n" : "") A_LoopFileFullPath

 	   If (StrLen(workimgs) > 0) {
			FileOpen(tess.infilePath, "w", "CP0").Write(workimgs)
		}
		else {
  ...
  ; Dateien kopieren
	...
	; Daten in Logdatei schreiben	    	;{
		...
		LogText := ";" PDFName ";time:" OCRDuration ";pages:" PageMax
					.  	(!ocrfailure ?	";filesize:" (!PDFSize ? 0 : PDFSize) ";textlength:" (!TXTSize ? 0 : TXTSize)
					:	"; ocr failure: `n" RegExReplace(RegExReplace(stdout, "[\n\r]", "|"), "[\t]", " "))
...
UCLCRatio(txt)                                                                      	{              	;-- Großbuchstaben/Kleinbuchstabenverhältnis

	; Tesseract scheint mehr Großbuchstaben zu erkennen umso schlechter die Qualität der zu scannenden Datei ist
	; eine Ratio zwischen 0.09 und 0.25 ist gut zu lesen, ab 0.35 wird es sehr schwierig, ab 1.00 kann man von einer verdrehten Vorlagen (Text auf dem Kopf ausgehen)
