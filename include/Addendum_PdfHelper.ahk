...
;   	by Ixiko started in September 2017 - last change 18.09.2022 - this file runs under Lexiko's GNU Licence
;
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ListLines, Off
return
...
PDFisCorrupt(pdfFilePath)                                                	{                	;-- überprüft ob die PDF Datei defekt ist
	...
	VarSetCapacity(EndOfFile, 7)
	fobj.seek(fobj.Length - 7, 1)
...
IFilter(file, searchstring:="", IFilterNr:=1)                        	{               	;-- Textsuche/Textextraktion in PDF/Word Dokumenten
	...
	/*
	                                                             
	IFILTER_INIT_CANON_PARAGRAPHS				Value: 1
	                                                             
		Absatzumbrüche sollten mit dem Unicode PARAGRAPH SEPARATOR (0x2029) gekennzeichnet werden.

	                                                             
	IFILTER_INIT_HARD_LINE_BREAKS 					Value: 2
	                                                             
		Weiche Zeilenumbrüche, wie z.B. das Newline-Zeichen in Word, sollten durch harte Zeilenumbrüche ersetzt werden?LINE SEPARATOR (0x2028). Vorhandene harte Zeilenumbrüche können verdoppelt werden.
		Ein Wagenrücklauf (0x000D), Zeilenvorschub (0x000A),	oder Wagenrücklauf und Zeilenvorschub in Kombination sollten als harter Zeilenumbruch betrachtet werden. Die Absicht ist, Musterausdrucke zu ermöglichen
		, die auf beobachtete Zeilenumbrüche passen.

	                                                             
	IFILTER_INIT_CANON_HYPHENS					Value: 4
	                                                             
		In verschiedenen Textverarbeitungsprogrammen gibt es Formen von Bindestrichen, die nicht im Host-Zeichensatz enthalten sind, wie z. B. optionale Bindestriche (die nur am Ende einer Zeile erscheinen)
		und nicht durchbrochene Bindestriche.  Dieses Flag zeigt an, dass optionale Bindestriche in Nullen und nicht durchbrochene Bindestriche in normale Bindestriche (0x2010) oder HYPHEN-MINUSES (0x002D)
		umgewandelt werden sollen.

	                                                             
	IFILTER_INIT_CANON_SPACES						Value: 8
	                                                             
		So wie das Flag IFILTER_INIT_CANON_HYPHENS die Bindestriche standardisiert, standardisiert dieses Flag die Leerzeichen. Alle speziellen Leerzeichen, wie z. B. nicht unterbrochene Leerzeichen, werden in das
		Standard-Leerzeichen (0x0020) umgewandelt.

	                                                             
	IFILTER_INIT_APPLY_INDEX_ATTRIBUTES 		Value: 16
	                                                             
		Gibt an, dass der Client möchte, dass Text in Blöcke aufgeteilt wird, die interne Werttypeigenschaften darstellen.

	                                                             
	IFILTER_INIT_APPLY_OTHER_ATTRIBUTES 		Value: 32
	                                                             
		Alle Eigenschaften, die nicht von den Flags IFILTER_INIT_APPLY_INDEX_ATTRIBUTES und IFILTER_INIT_APPLY_CRAWL_ATTRIBUTES abgedeckt sind, sollten ausgegeben werden.

	                                                             
	IFILTER_INIT_INDEXING_ONLY	              		Value: 64
	                                                             
	Optimiert IFilter für die Indizierung, da der Client die Methode IFilter::Init nur einmal und nicht IFilter::BindRegion aufruft. Damit entfällt die Möglichkeit, auf einen Chunk sowohl vor als auch nach dem Zugriff auf einen
		anderen Chunk zuzugreifen.

	                                                             
	IFILTER_INIT_SEARCH_LINKS	                		Value: 128
	                                                             
		Der Textextraktionsprozess muss alle verknüpften Objekte innerhalb des Dokuments rekursiv durchsuchen. Wenn ein Link nicht verfügbar ist, sollte der IFilter::GetChunk-Aufruf, der den ersten Chunk des Links erhalten
		hätte, FILTER_E_LINK_UNAVAILABLE zurückgeben.

	                                                             
	IFILTER_INIT_APPLY_CRAWL_ATTRIBUTES		Value: 256
	                                                             
		Gibt an, dass der Client möchte, dass Text in Blöcke aufgeteilt wird, die Eigenschaften darstellen, die während des Indizierungsprozesses bestimmt wurden.

	                                                             
	IFILTER_INIT_FILTER_OWNED_VALUE_OK		Value: 512
	                                                             
		Der Prozess der Inhaltsindizierung kann Eigenschaftswerte zurückgeben, die durch den Filter festgelegt wurden.

	                                                             
	IFILTER_INIT_FILTER_AGGRESSIVE_BREAK		Value: 1024
	                                                             
	TBD

	                                                             
	IFILTER_INIT_DISABLE_EMBEDDED					Value: 2048
	                                                             
	TBD

	                                                             
	IFILTER_INIT_EMIT_FORMATTING					Value: 4096
	                                                             
	TBD
	 */
	...
	static MY_IFILTER, myIFinit

	if !myIFinit {

		myIFinit := true
		MY_IFILTER := []
		IFltr_Base := IFILTER_INIT_CANON_PARAGRAPHS | IFILTER_INIT_HARD_LINE_BREAKS | IFILTER_INIT_CANON_HYPHENS | IFILTER_INIT_CANON_SPACES

		IFltr := IFltr_Base  | IFILTER_INIT_EMIT_FORMATTING
		MY_IFILTER.Push(IFltr)

		IFltr := IFltr_Base | IFILTER_INIT_APPLY_OTHER_ATTRIBUTES
		MY_IFILTER.Push(IFltr)

		IFltr := IFltr_Base | IFILTER_INIT_EMIT_FORMATTING | IFILTER_INIT_DISABLE_EMBEDDED
		MY_IFILTER.Push(IFltr)

		IFltr := IFltr_Base | IFILTER_INIT_EMIT_FORMATTING | IFILTER_INIT_APPLY_INDEX_ATTRIBUTES
		MY_IFILTER.Push(IFltr)

		IFltr := IFltr_Base | IFILTER_INIT_EMIT_FORMATTING | IFILTER_INIT_APPLY_CRAWL_ATTRIBUTES
		MY_IFILTER.Push(IFltr)
	...
	if (DllCall(NumGet(NumGet(IFilter+0)+3*A_PtrSize), "Ptr", IFilter, "UInt", MY_IFILTER[IFilterNr], "Int64", 0, "Ptr", 0, "Int64*", status) != 0 ) ; IFilter::Init
