...
;------------------------------------------------------------------------------------------------------------------------------------
;------------------------------------------------------------------------------------------------------------------------------------
;------------------------------------- written by Ixiko -this version is from 18.05.2019 ------------------------------------
;------------------------------------------------------------------------------------------------------------------------------------
...
;--> begonnen 	: 45. Befundordner soll eher eine Datenbank werden (dazu verschieben neu gescannter Befunde in einen anderen Ordner und erst bei Neuaufruf des Skriptes in den normalen dann Datenbank geführten Befundordner)  (V0.9.86)
...
;| **04.05.2019** | **F+** | PdfAddMetaDataInfo() - nutzt die pdftk.exe commandline exe, es lassen sich Metainformation schreiben, welche die xpdfinfo.exe leider nicht ausliest (V0.9.88) |
;| **14.05.2019** | **F~** | Fuzzy Matching von Patientenfenster Albis und Pdf Dateinnamen funktioniert jetzt super! |
;| **14.05.2019** | **F~** | PdfReader_Close() - ist fehlerfrei und schnell |
...
;| **18.05.2019** | **F+** | Albis Pdf's - automatisches Schließen der von Albis nach import geöffneten Foxitreaderfenster (V0.9.89) |
;| **18.05.2019** | **F+** | **ExtractNamesFromFileName()** - entfernt Dr. und Prof. am Namensanfang |
;| **20.05.2019** | **F-** | die Automatisierung des Pdf-Importes und des Foxitreader erfolgt nun fast ausschließlich über Funktionsaufrufe und WinEventHooks, das Label BoSaveSignedPdf wurde entfernt |
;}
...
;67. Fenster			- Pat. hat in diesem Quartal seine Chipkarte ... wird nicht zuverlässig erkannt
...
;{ 2. Variblen Setup / Registry auslesen
	...
	;{ a) ---------------------------------------------------------------- globale Variablen -----------------------------------------------------------------------
		...
		global AutoCloseReader                    			;der Signaturprozeß setzt diesen auf 1
	...
	;{ d) ------------------------------------------------------------------ Hinweisdialoge -------------------------------------------------------------------------
        	...
        	Hinweis5 =
			(LTrim
			Die eingestellte Signatur: `"#####`" ist auf diesem Computer
...
;}
	...
	;{ m) ---------------------------------------------------------- Patientendatenbank einlesen ----------------------------------------------------------------
			...
			oPat:= ReadPatientDatabase(SPData.AddendumDBPath)
...
return
	...
	;}
_BOEditItem:                                   	;{		Datei umbenennen

		Running:="Dokument`numbenennen`n"

	;Überprüfen ob die Datei per PDF-Reader geöffnet ist, wenn ja muss dieser zunächst geschlossen werden
		while WinExist(CRowText " ahk_class " PDFReaderWinClass)
		{
					PdfReader_Close(CRowText, PDFReaderWinClass)
					If WinExist(CRowText " ahk_class " PDFReaderWinClass)
					{
...
BOUebertragen:                             	;{ 	übertragen eines Befundes in die Akte
	...
	; selektierte Dateien nach und nach übertragen
		For key, selected in SelectedPdfs
		{
				...
				; erster Durchlauf
					;If A_Index = 1
					;	FileAppend, % "`n", % AddendumDBPath "\PdfPfade.csv"

				; löschen des Eintrages aus dem ScanPool[] Array
					PageSum -= ScanPoolArray("Delete", PdfData["PdfFullPath"])
				...
				; löschen des Pdf-Eintrages aus dem Listview
					Gui, BO: Default
					pdftodelete:= StrReplace(PdfFullPath, ".pdf", "")
					;dRow:= LV_EX_FindString(hLV1, pdftodelete, 0, False)
					;LV_Delete(dRow)
...
;{ 7. PDF Suche | Pdf Preview | ProcessToolTip
...
;{ 8. FoxitReader Automation Functions
...
FoxitReader_CloseSaveAs(hHook2) {																		;-- zum Schliessen des Dialogfenster "Speichern unter" (z.B. als Hookhandler)
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		hHook	:= GetHex(hHook2)
		hHook2:= GetHex(hHook2)
		HClass	:= GetClassName(hHook2)
		HTitle	:= WinGetTitle(hHook2)
		hParent	:= GetHex(GetParent(hHook2))
		PClass	:= GetClassName(hParent)
	;}

	;----------------------------------------------------------------------------------------------------------------------------------------------
	; b) PDF-Dateinamen aus dem FoxitReader Fenstertitel generieren
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		ftitle		:= WinGetTitle(hParent)
		RegExMatch(ftitle, "i).*(?=\.pdf)", fname)                                         	;Dateiname aus dem FoxitReader Fenster

		while !InStr(PClass, "classfoxitReader")
		{
				hParent:= GetParent(hParent)
				If !hParent
				{
					MsgBox, 1, Addendum für Albis on Windows, % "ScanPool kann den Titel des FoxitReader`nFenster nicht ermitteln"
				}
				else
				{
					RegExMatch(ftitle:=WinGetTitle(hParent), "i).*(?=\.pdf)", "")
					continue
				}
		...
		headway := "1. neues PdfReader Fenster wurde erkannt"
	;}

	;----------------------------------------------------------------------------------------------------------------------------------------------
	; c) Auslesen von Dateiordner und PDF-Dateinamen aus den Edit-Controls auslesen
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		WinGetPos, wx, wy, ww, wh, ahk_id %hHook2%
		MouseMove, % wx + ww + 10, % wy, 0

		efname := ControlCmd("Edit1", "GetText", hHook2)                             	;eingetragener Dateiname im Dialogfenster
						 ControlCmd("ToolBarWindow324", "Click use ControlClick Left", hHook2)
		If !fDir	:= ControlCmd("Edit2", "GetText", hHook2) {
						 ControlCmd("ToolBarWindow324", "Click use MouseClick Left", hHook2)
			fDir	:= ControlCmd("Edit2", "GetText", hHook2)
		}
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		If !InStr(efname, fname)
				ControlSetTextEx("", fname, "ahk_id " ControlCmd("Edit1"	, "hwnd", hHook2) , 200)
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		...
		{
					propWasSet:=1
					If !ControlCmd("Edit2", "GetText", hHook2)			;falls Edit2 nicht mehr freigeschaltet ist
						 ControlCmd("ToolBarWindow324", "Click use MouseClick"	, hHook2)
					ControlCmd("Edit2", "SetText " BefundOrdner	, hHook2)
					ControlCmd("Edit2", "Send {Enter}"				, hHook2)
		}
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		ControlClassNN:= ControlCmd("", "FindByText(Speichern, Button)", hHook2)				;Deutsch
		If !ControlClassNN
				ControlClassNN:= ControlCmd("", "FindByText(Save, Button)", hHook2)					;Englisch
		If ControlClassNN {
				res:= ControlCmd(ControlClassNN, "SetFocus"	, hHook2)
				res:= ControlCmd(ControlClassNN, "Click"		, hHook2)		;ControlClassNN sollte jetzt Button2 oder Button3 als Text enthalten
				headway .= "`n5. Speichern Schaltfäche -" ControlClassNN "- wurde gedrückt. "
		} else {
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------
	; f) Löschen des ControlCmd-Objekts, automat. Schließen eines gerade signierten Pdf-Befundes
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		headway .= "`n6. Warte auf das Schließen des Dialoges`nim Anschluß wird das Dokumentfenster geschlossen."
		;PraxTT(headway, "0 1")
		hSpu:= WinExist("Speichern unter ahk_class #32770")

	;geöffnetes Pdf-Dokument nach dem Signieren schliessen
		If AutoCloseReader {

				while, WinExist("ahk_id " hSpu)
				{
						if hHook1:= WinExist("Speichern unter bestätigen ahk_class #32770")
									FoxitReader_ConfirmSaveAs(hHook1)
						Sleep, 30
				}

			; dies ermöglich das Schließen des FoxitReader-Fensters noch abbrechen zu können im Ausnahmefall
				If AutoCloseReader
				{
						PdfReader_Close(fname, PDFReaderWinClass)
						AutoCloseReader:= 0
				}
		}

	;Infofenster ausschalten
		;SetTimer, PraxTToff, -30
		fError:= ControlCmd("", "Reset", hHook2)
		hHook := 0
	;}
...
FoxitReader_ExceptionDialog(hHook3) {                                                                	;-- zum Schließen eines selten vorkommenden Dialogfensters
	; verhindert das der FoxitReader geschlossen wird, da jetzt ein manueller Eingriff notwendig ist.
		AutoCloseRreader:= 0
return SureControlClick("Button1", "", "", hHook3)
}
...
FoxitReader_SignDoc(hDokSig, FoxitTitle, FoxitText:="") {											;-- Winhook-Handler zum Bearbeiten des Dokument signieren Dialoges
	...
	;{ Einstellungen für das Fenster 'Dokument signieren'
			...
			If !Ort
				Ort:= IniRead(AddendumDir "\Addendum.ini", "ScanPool", "Ort")
			If !Grund
				IniRead, Grund												, %AddendumDir%\Addendum.ini, ScanPool, Grund
			If !SignierenAls
				IniRead, SignierenAls										, %AddendumDir%\Addendum.ini, ScanPool, SignierenAls
	...
	;{ Felder im Signierfenster auf die in der INI festgelegten Werte einstellen
			...
			;Darstellungstyp ----------------------------------------------------------------------------------------------------
				...
				ControlGet, entryNr, FindString	, % StrReplace(Darstellungstyp, Chr(0x22), ""), ComboBox4, % "ahk_id " hDokSig  ;StrReplace plötzlich waren 2 " um den String in der ini
				If !entryNr
				{
						MsgBox, 4144, Addendum für AlbisOnWindows - ScanPool, % StrReplace(Hinweis5, "#####", Darstellungstyp)
						ControlGet, entryString, Choice,	, ComboBox4, % "ahk_id " hDokSig
						ListVars
						MsgBox, 4145, Addendum für AlbisOnWindows - ScanPool, % "Sie haben folgendes ausgewählt: " entryString "`nMöchten Sie absofort diese Signatur nutzen?"
						IfMsgBox, Yes
								IniWrite, % entryString, %AddendumDir%\Addendum.ini, ScanPool, Darstellungstyp
				}
				else
				{
						Control, ChooseString, % Darstellungstyp, ComboBox4, % "ahk_id " hDokSig
...
PdfReader_Close(ReaderTitleOrID, PDFReaderWinClass, CloseTabOnly:=0) {			;-- schließen eines FoxitReader-Tabs oder Beenden einer PDF-Reader Instanz
	...
	If RegExMatch(ReaderTitleOrID, "0x") or RegExMatch(ReaderTitleOrID, "^\d+$") {
			ReaderID:= ReaderTitleOrID
	} else if RegExMatch(ReaderTitleOrID, "\w") {
			ReaderID:= WinExist(ReaderTitleOrID " ahk_class " PDFReaderWinClass)
	}
  ...
  ;--------------------------------------------------------------------- Befehle -----------------------------------------------------------------------------------
	...
	If Instr(cmd, "Delete")	            		;param: gesuchter Dateiname										, Rückgabe: Wert       	- Seitenanzahl der entfernten Pdf-Datei
	{
			For key, val in ScanPool
			{
					If Instr(val, param)
					{
						FileAppend, TimeCode(1) "| Datei: " param " aus dem Verzeichnis gelöscht.`n", % AddendumDir . "\logs'n'data\ScanPoolArrayLog.txt"
...
;---------------------------------------ALBISHANDLING                                                 	;{
FuzzyNameMatch(Name1, Name2) {                                                                 	;-- Fuzzy Suchfunktion

	surname1	:= Trim(Name1[1])
	prename1	:= Trim(Name1[2])
	surname2	:= Trim(Name2[3])
	prename2	:= Trim(Name2[4])
	if ( StrDiff(surname1 . prename1, surname2 . prename2) <= 0.15 )  ||  ( StrDiff(surname1 . prename1, prename2 . surname1) <= 0.15 )
			return 1
...
AkteOeffnen(Pat, PdfPath:="") {                                                                          	;-- Akte lässt sich auch öffnen, selbst wenn der Name nicht ganz korrekt geschrieben ist
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		...
		If AllreadyOpen
		{
				AlbisOeffneKarteikarte() 		; sicherstellen das die Karteikarte angezeigt wird
	...
	; 32768 Menu Kommando für den 'Patient öffnen' Auswahldialog
		...
		If ControlSetTextEx("Edit1", (Pat[5]="" ? Pat[1] : Pat[5]), "Patient öffnen ahk_class #32770", 200)			;Pat enthält nur die ID wenn es kein Array ist
		{
...
AlbisPdfUebertragen(PdfFullPath, LVRow) {                                                         	;-- Übertragen eines Befundes in eine Patientenakte
	...
	; Eingangsdatum des Befundes auslesen
		creationtime:= FormatedFileCreationTime(SPData["BefundOrdner"] "\" PdfFullPath)

	; Prüfen ob der Patient in der Datenbank vorhanden ist und ermitteln einer Patienten ID wenn möglich
		If oPat["MaxPat"] {
				If suggestions:= PatInDB(oPat, Name)
				{
	...
	; Erstellen des Karteikartentextes
		...
		;KarteikartenText 	:= StrReplace(KarteikartenText, Name[3], "(" Name[3] ", ")
		;KarteikartenText 	:= StrReplace(KarteikartenText, Name[4], Name[4] ") - ")
	...
	; Übertragen des Befundes in die Akte
		success:= AlbisUebertrageGrafischenBefund(PdfFullPath, KarteikartenText)
		If !success {
	...
	; Daten sichern (PdfData ist global - ein return nicht notwendig)
		...
		PdfData.PdfFullPath 			:= PdfFullPath
...
MetaDatenAnzeigen(PdfPath)	{																;-- zeigt die Metadaten einer PDF Datei an
...
SpWinProc1(hWinEventHook, Event, SpHwnd1, idObject, idChild, eventThread, eventTime) {				;Hookverteiler
		Critical
		Sleep,50
		If GetHex(SpHwnd1) = SpEHookHwnd1
				return 0
		wclass:= WinGetClass(SpEHookHwnd1:= GetHex(SpHwnd1))
		If wclass in #32770,AutoHotkeyGUI,classFoxitReader,OptoAppClass
		{
				SetTimer, SpEvHook_WinHandler1,  -0
				return 0
		}
		SpProc1:= WinGet(SpEHookHwnd1, "ProcessName")
		If SpProc1 contains albis,%PdfReader%,Autohotkey
				SetTimer, SpEvHook_WinHandler1,  -0
return 0
}
...
SpEvHook_WinHandler1:                     	;{
		...
		SpProc1 	:= WinGet(SpEHookHwnd1, "ProcessName")
			   ...
			   if InStr(SpProc1, "Foxitreader") {
					...
					FoxitText1	:= WinGetText(SpEHookHwnd1)
					...
					else if InStr(FoxitText1, "Do you want to save changes to")
								SureControlClick("Button1", "FoxitReader", "Do you want to save changes to", "")
		}
				...
				If RegExMatch(albisPopUpText, "i)Patient.*nicht vorhanden" )
							...
							SureControlClick("Button1", "ALBIS", "Patient hat in diesem Quartal")
				else if InStr(albisPopUpTitle, "Herzlichen Glückwunsch zum Geburtstag")
							SureControlClick("Button1", "Herzlichen Glückwunsch zum Geburtstag", "")
		}
...
BuildScanPoolDataObject() {
	  ...
	  ; pdftk.exe - für Schreiben von neuen Metadaten in die PDF-Datei (Patientendaten)
		...
		inistrings.= AddendumDir "\Addendum.ini, ScanPool	, SignatureClients		,"
