...
; 10.05.2019 PdfAddMetaDataInfo() -  (V0.9.88)
; 14.05.2019 Fuzzy Matching von Patientenfenster Albis und Pdf Dateinnamen funktioniert jetzt super!
; 14.05.2019 PdfReader_Close() - ist fehlerfrei und schnell
; 15.05.2019 Funktionen zur Behandlung des Speichern unter Systemdialoges in Windows10, FoxitReader_ConfirmSaveAs(), FoxitReader_CloseSaveAs(), FoxitReader_ExceptionDialog()
;--> erledigt		: 62. Albis Pdf's		- automatisches Schließen der von Albis nach import geöffneten Foxitreaderfenster (V0.9.89)
; 18.05.2019 **ExtractNamesFromFileName()** - entfernt Dr. und Prof.
;}
...
BOUebertragen:                             	;{ 	übertragen eines Befundes in die Akte
	...
	; alles zurücksetzen - fertig
		...
		WinActivate, % PDFReaderWinClass
		WinWaitActive, % PDFReaderWinClass,, 3
...
FoxitReader_CloseSaveAs(hHook2) {																		;-- zum Schliessen des Dialogfenster "Speichern unter" (z.B. als Hookhandler)
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		...
		headway := "1. neues PdfReader Fenster wurde erkannt:`nPTitle: " PTitle "`nPClass: " PClass "`nHClass: " HClass
	;}

	;----------------------------------------------------------------------------------------------------------------------------------------------
	; c) Auslesen von Dateiordner und PDF-Dateinamen aus den Edit-Controls auslesen
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		efname := ControlCmd("Edit1", "GetText", foxitHook)
						 ControlCmd("ToolBarWindow324", "Click use ControlClick Left", foxitHook)
		If !fDir	:= ControlCmd("Edit2", "GetText", foxitHook) {
						 ControlCmd("ToolBarWindow324", "Click use MouseClick Left", foxitHook)
			fDir	:= ControlCmd("Edit2", "GetText", foxitHook)
		}
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		If !InStr(efname, fname)
				ControlSetTextEx("", fname, "ahk_id " ControlCmd("Edit1"	, "hwnd", foxitHook) , 200)
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		...
		{
					propWasSet:=1
					If !ControlCmd("Edit2", "GetText", foxitHook)			;falls Edit2 nicht mehr freigeschaltet ist würde ein leerer String zurückgegeben
						 ControlCmd("ToolBarWindow324", "Click use MouseClick Left"	, foxitHook)
					ControlCmd("Edit2", "SetText " BefundOrdner	, foxitHook)
					ControlCmd("Edit2", "Send {Enter}"				, foxitHook)
		}
	...
	;----------------------------------------------------------------------------------------------------------------------------------------------;{
		ControlClassNN:= ControlCmd("", "FindByText(Speichern, Button)", foxitHook)				;Deutsch
		If !ControlClassNN
				ControlClassNN:= ControlCmd("", "FindByText(Save, Button)", foxitHook)					;Englisch
		If ControlClassNN {
				res:= ControlCmd(ControlClassNN, "SetFocus"	, foxitHook)
				res:= ControlCmd(ControlClassNN, "Click"		, foxitHook)		;ControlClassNN sollte jetzt Button2 oder Button3 als Text enthalten
	...
	;geöffnetes Pdf-Dokument nach dem Signieren schliessen
		...
		foxitHook 	:= 0
		;FileAppend, % headway "`n`n", %A_ScriptDir%\SaveAsLog.txt
	;}
...
FoxitReader_SignDoc(hDokSig, FoxitTitle, FoxitText:="") {											;-- Winhook-Handler zum Bearbeiten des Dokument signieren Dialoges
	...
	PraxTT("Die Bearbeitung des Fenster 'Dokument signieren' ist abgeschlossen!'", "3 2")
	Sleep, 300
...
FuzzyNameMatch(Name1, Name2) {                                                                 	;-- Fuzzy Suchfunktion
	...
	if ( StrDiff(surname1 . prename1, surname2 . prename2) <= 0.2 )  ||  ( StrDiff(surname1 . prename1, prename2 . surname2) <= 0.2 )
			return 1
