...
; Changelog:
;     1.0.00.00/2015-08-23/just me     - initial release
; Common Parameters:
...
TVH_GetHovered(HTV) {
   ...
   Return % (Result & 0x0046 ? Item : 0) ; TVHT_ONITEM
}
; ====================================================================================================
...
TVH_GetItem(HTV, ItemID) { ; TVM_GETITEMW : TVM_GETITEMA
   ...
   If !DllCall("GetWindowThreadProcessId", "Ptr", HTV, "UIntP", TVPID, "UInt")
      Return 0
   If (TVPID <> TVH_AhkPID()) {
      If !(RMO := RMO_Create(TVPID, 8192, False)) ; should be enough in any rate
         Return 0
      Remote := True
      , RMO.Init()
      , Is32Bit := RMO_IsProcess32Bit(TVPID)
      , PtrType := Is32Bit ? "UInt" : "Int64"
      , PtrSize := Is32Bit ? 4 : 8
   }
   Else {
      PtrSize := A_PtrSize
   ...
   SizeOfTVIX := 40 + (PtrSize * 5)
   , VarSetCapacity(TVIX, SizeOfTVIX, 0) ; TVITEMEX
   , NumPut(0x00FF, TVIX, 0, "UInt") ; mask
   , NumPut(ItemID, TVIX, PtrSize, PtrType) ; hItem
   , NumPut(Remote ? RMO.Addr + SizeOfTVIX : &ItemText, TVIX, (PtrSize * 2) + 8, PtrType) ; pszText
   , NumPut(SizeOfText, TVIX, (PtrSize * 3) + 8, "UInt") ; cchTextMax
   ...
   If DllCall("SendMessage", "Ptr", HTV, "UInt", A_IsUnicode ? 0x113E : 0x110C, "Ptr", 0, "Ptr", PtrTVIX, "UInt") {
      If (Remote) {
         RMO.Get(TVIX, 0, SizeOfTVIX)
         RMO.Get(ItemText, SizeOfTVIX, SizeOfText)
      }
      VarSetCapacity(ItemText, -1)
      Return {State: NumGet(TVIX, PtrSize * 2, "UInt"), Text: ItemText, Icon: NumGet(TVIX, (PtrSize * 3) + 12, "Int") + 1
            , Children: NumGet(TVIX, (PtrSize * 3) + 20, "Int"), Param: NumGet(TVIX, (PtrSize * 3) + 24, PtrType)}
   }
   Return False
}
; ====================================================================================================
...
; ====================================================================================================
TVH_GetItemCount(HTV, ItemID:=0) { ; TVM_GETCOUNT
   Return DllCall("SendMessage", "Ptr", HTV, "UInt", 0x1105, "Ptr", 0, "Ptr", ItemID, "Int")
}
; ====================================================================================================
...
TVH_GetItemRect(HTV, ItemID, ByRef X, ByRef Y, ByRef W, ByRef H, TextOnly := True) { ; TVM_GETITEMRECT
   ...
   If !DllCall("GetWindowThreadProcessId", "Ptr", HTV, "UIntP", TVPID, "UInt")
      Return 0
   If (TVPID <> TVH_AhkPID()) {
      If !(RMO := RMO_Create(TVPID, 16, False))
         Return 0
      Remote := True
      , RMO.Init()
      , PtrType := RMO_IsProcess32Bit(TVPID) ? "UInt" : "Int64"
   }
   Else
      PtrType := "UPtr"
   VarSetCapacity(RC, 16, 0)
   , NumPut(ItemID, RC, 0, PtrType)
   If (Remote)
      RMO.Put(RC, 0, 16)
   If DllCall("SendMessage", "Ptr", HTV, "UInt", 0x1104, "Ptr", !!TextOnly, "Ptr", Remote ? RMO.Addr : &RC, "Int") {
      If (Remote)
         RMO.Get(RC, 0, 16)
      X := NumGet(RC, 0, "Int")
      , Y := NumGet(RC, 4, "Int")
...
TVH_GetNextChecked(HTV, ItemID := 0) {
   ...
   While (NextChecked = 0) && (ItemID <> 0) {
      ...
      Else
         ItemID := TVH_GetNextSibling(HTV, ItemID)
...
TVH_GetParam(HTV, ItemID) { ; TVM_GETITEMW : TVM_GETITEMA
   ...
   If (Remote)
      RMO.Get(TVIX, 0, SizeOfTVIX)
   Return NumGet(TVIX, 24 + (PtrSize * 3), PtrType)
}
; ====================================================================================================
...
TVH_HitTest(HTV, ByRef Result, X := "", Y := "") { ; TVM_HITTEST
   ...
   If (TVPID <> TVH_AhkPID()) {
      If !(RMO := RMO_Create(TVPID, 32, False))
         Return 0
      Remote := True
      , RMO.Init()
      , PtrSize := RMO_IsProcess32Bit(TVPID) ? 4 : 8
   }
   Else
      PtrSize := A_PtrSize
   SizeOfTVHTI := 8 + (PtrSize * 2)
   VarSetCapacity(TVHTI, SizeOfTVHTI, 0)
   NumPut(X, TVHTI, 0, "Int")
   NumPut(Y, TVHTI, 4, "Int")
   If (Remote)
      RMO.Put(TVHTI, 0, SizeOfTVHTI)
   Item := DllCall("SendMessage", "Ptr", HTV, "UInt", 0x1111, "Ptr", 0, "Ptr", Remote ? RMO.Addr : &TVHTI, "UPtr")
   If (Remote)
      RMO.Get(TVHTI, 0, SizeOfTVHTI)
...
TVH_Insert(HTV, ItemText, ParentID := 0, InsertAfter := "Last", Options := "") {
   ...
   , SizeOfTVINS := 40 + (PtrSize * 7)
   , VarSetCapacity(TVINS, SizeOfTVINS, 0) ; TVINSERTSTRUCT
   , NumPut(ParentID, TVINS, 0, PtrType) ; hParent
   , NumPut(InsertAfter, TVINS, PtrSize, PtrType) ; hInsertAfter
   , NumPut(Mask, TVINS, (PtrSize * 2), "UInt") ; mask
   , NumPut(State, TVINS, (PtrSize * 4), "UInt") ; state
   , NumPut(State, TVINS, (PtrSize * 4) + 4, "UInt") ; stateMask
   , NumPut(Remote ? RMO.Addr + SizeOfTVINS : &TVINS, TVINS, (PtrSize * 4) + 8, PtrType) ; pszText
   , NumPut(Icon, TVINS, (PtrSize * 5) + 12, "Int") ; iImage
   , NumPut(Icon, TVINS,  (PtrSize * 5) + 16, "Int") ; iSelectedImage
   , PtrTVINS := Remote ? RMO.Addr : &TVINS
   If (Remote) {
      RMO.Put(TVINS, 0, SizeOfTVINS)
      , RMO.Put(ItemText, SizeOfTVINS, SizeOfText)
   }
...
TVH_SetParam(HTV, ItemID, Param) { ; TVM_SETITEMW : TVM_SETITEMA
   ...
   , NumPut(0x0004, TVIX, 0, "UInt") ; mask = TVIF_PARAM
   , NumPut(ItemID, TVIX, PtrSize, PtrType) ; hItem
   , NumPut(Param, TVIX, 24 + (PtrSize * 3), PtrType)
   , PtrTVIX := Remote ? RMO.Addr : &TVIX
   If (Remote)
      RMO.Put(TVIX, 0, SizeOfTVIX)
...
TVH_SetText(HTV, ItemID, ItemText) { ; TVM_SETITEMW : TVM_SETITEMA
   ...
   , NumPut(ItemID, TVIX, PtrSize, PtrType) ; hItem
   , NumPut(Remote ? RMO.Addr + SizeOfTVIX : &ItemText, TVIX, (PtrSize * 2) + 8, PtrType)
   , RMO.Put(TVIX, 0, SizeOfTVIX)
...
; ====================================================================================================
TVH_AhkPID() {
   Static AhkPID := DllCall("GetCurrentProcessId", "UInt")
   Return AhkPID
}
; ----------------------------------------------------------------------------------------------------------------------------------
TVH_OSVersion() {
