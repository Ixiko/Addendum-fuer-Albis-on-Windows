...
; Language:    	English
; Version:      	1.0.02.00/2022-01-04/ixiko (add custom selection color)
...
;                         TS 	- default text selection color.
;                         B 	- default background color.
;                         BS 	- default background selection color.
...
Class OD_Colors {
   ...
   Attach(HWND, Colors) {
      ...
      For Key, Value In Colors {
		 ...
		 If (Key = "TS") {
            This.Controls[HWND].Colors.TS 	:= ((Value & 0xFF) << 16) | (Value & 0x00FF00) | ((Value >> 16) & 0xFF)
            Continue
         }
         If (Key = "B") {
		  ...
		  If (Key = "BS") {
            This.Controls[HWND].Colors.BS 	:= ((Value & 0xFF) << 16) | (Value & 0x00FF00) | ((Value >> 16) & 0xFF)
            Continue
         }
         If ((Item := Round(Key)) = Key) {
            ...
            If ((C := Value.TS) <> "")
               This.Controls[HWND].Colors[Item, "TS"] := ((C & 0xFF) << 16) | (C & 0x00FF00) | ((C >> 16) & 0xFF)
            If ((C := Value.B) <> "")
               This.Controls[HWND].Colors[Item, "B"] := ((C & 0xFF) << 16) | (C & 0x00FF00) | ((C >> 16) & 0xFF)
            If ((C := Value.BS) <> "")
               This.Controls[HWND].Colors[Item, "BS"] := ((C & 0xFF) << 16) | (C & 0x00FF00) | ((C >> 16) & 0xFF)
         }
      }
   ...
   DrawItem(lParam, Msg, Hwnd) { ; first param 'wParam' is passed as 'This'.
      ...
      If OD_Colors.Controls.HasKey(HWND) && ODT.HasKey(NumGet(lParam + 0, 0, "UInt")) {
		 ...
		 If (State & ODS_SELECTED) {
			BgC 	:= ODCtrl.Colors[Item].HasKey("BS") ? ODCtrl.Colors[Item].BS : CtrlBgC
			TxC 	:= ODCtrl.Colors[Item].HasKey("TS") ? ODCtrl.Colors[Item].TS : CtrlTxC
			DrawFocusRect := ODCtrl.Colors[Item].HasKey("BS") || ODCtrl.Colors[Item].HasKey("TS") ? false : true
		 }
		 else {
         ...
         NumPut(NumGet(RECT + 0, 0, "Int") - 2, RECT + 0, 0, "Int")
         If (State & ODS_SELECTED) && DrawFocusRect
