/*
	...
	GetCurrentURL(fromAddressBar=False)
		Gets the current URL. fromAddressBar=True gets it straight from the URL bar element, which is not a very good method, because the text might be changed by the user and doesn't start with "http(s)://".
		Default of fromAddressBar=False will cause the real URL to be fetched, but the browser must be visible for it to work (if is not visible, it will be automatically activated).
	...
	JSGetElementPos(selector, useRenderWidgetPos=False)
		Uses Javascript's querySelector to get a Javascript element and then its position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position element relative to the window,
		otherwise it uses UIA_Browsers CurrentDocumentElement position.
	...
	ControlClickJSElement(selector, WhichButton="", ClickCount="", Options="", useRenderWidgetPos=False)
		Uses Javascript's querySelector to get a Javascript element and then ControlClicks that position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position element relative to the window,
		otherwise it uses UIA_Browsers CurrentDocumentElement position.
	ClickJSElement(selector, WhichButton="", ClickCount=1, DownOrUp="", Relative="", useRenderWidgetPos=False)
		Uses Javascript's querySelector to get a Javascript element and then Clicks that position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position element relative to the window,
		otherwise it uses UIA_Browsers CurrentDocumentElement position.
*/
...
class UIA_Browser {
	...
	InitiateUIA(wTitle:="A", customNames:="", maxVersion:="") {
		If !this.BrowserId
			If !(this.BrowserId := this.GetBrowserId(wTitle))
				return -77
		...
		try
			...
			return e		; sometimes Edge hangs up, this will give a chance to react
		this.GetCurrentMainPaneElement()
	}
	; 	Initiates UIA and hooks to the browser window specified with wTitle. customNames can be an object that defines custom CurrentName values for locale-specific elements (such as the name of the URL bar):
	;  {URLEditName:"My URL Edit name", TabBarName:"Tab bar name", HomeButtonName:"Home button name", StopButtonName:"Stop button", NewTabButtonName:"New tab button name"}.
	; 	maxVersion specifies the highest UIA version that will be used (default is up to version 7).
	__New(wTitle:="A", customNames:="", maxVersion:="") {
		local bt
		If !(this.BrowserId := this.GetBrowserId(wTitle))
		...
		if (bt != "Unknown") {
			...
			res := this.InitiateUIA(wTitle, customNames, maxVersion)
			If (IsObject(res) && res.What)
				return -99
			else if (res+0 < 0)
				return res
		}
	...
	GetBrowserId(wTitle) {
		...
		If !(BrowserId := WinExist(wTitle)) {

			atmm := A_TitleMatchMode

			SetTitleMatchMode, 2
			BrowserId := WinExist(wTitle)
		...
		SciTEOutput(A_ThisFunc "() | atmm: " atmm ", hwnd: " BrowserId)
	...
	; returns language used (only Edge)
	JSGetBrowserLanguage() {
		if (this.BrowserType != "Edge")
			return
		js := "(() => { return navigator.language;})()"

	return  this.JSReturnThroughClipboard(js)
	}
	...
	; Uses Javascript's querySelector to get a Javascript element and then its position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate
	; the position element relative to the window, otherwise it uses UIA_Browsers CurrentDocumentElement position.
	...
	; Uses Javascript's querySelector to get and click a Javascript element. Compared with ClickJSElement method, this method has the advantage of skipping the need to wait for a return value from the clipboard,
	; but it might be more unreliable (some elements might not support Javascript's "click()" properly).
	JSClickElement(selector) {
        this.JSExecute("document.querySelector(""" selector """).click();")
	}

	; Uses Javascript's querySelector to get a Javascript element and then ControlClicks that position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position
	; element relative to the window, otherwise it uses UIA_Browsers CurrentDocumentElement position.
    ControlClickJSElement(selector, WhichButton:="", ClickCount:="", Options:="", useRenderWidgetPos:=False) {
		local
	...
	; Uses Javascript's querySelector to get a Javascript element and then Clicks that position. useRenderWidgetPos=True uses position of the Chrome_RenderWidgetHostHWND1 control to locate the position
	; element relative to the window, otherwise it uses UIA_Browsers CurrentDocumentElement position.
    ClickJSElement(selector, WhichButton:="", ClickCount:=1, DownOrUp:="", Relative:="", useRenderWidgetPos:=False) {
		local
        bounds := this.JSGetElementPos(selector, useRenderWidgetPos)
