...
; Modifications by Ixiko
;	Mod. History: 2021-04-23 	- The width of the EditCell is can be displayed with the same width of the Listview cell you want to edit
; ======================================================================================================================
; CLASS LV_InCellEdit
;
...
Class LV_InCellEdit {
   ...
   ; PUBLIC INTERFACE
	   EditCell(Row, Col := 0) {
		  ...
		  ControlGet, Cols, List, Count Col, , % "ahk_id " This.HWND
	   ...
	   SetColumns(ColNumbers*) {
		   ...
		   ; Parameters:     ColNumbers* -  zero or more numbers of column which shall be editable. If entirely omitted, the ListView will be reset to enable editing of all columns.
		   ; Return values:  True on success; otherwise False
		   ; =================================================================================================
	...
	; PRIVATE METHODS
	   ...
	   LVN_BEGINLABELEDIT(L) {
		  ...
		  Static Indent := 1   ; indent of the Edit control, 4 seems to be reasonable for XP, Vista, and 7
		  If (This.Item = -1) || (This.SubItem = -1)
			 Return True
		  SendMessage, 0x1018, 0, 0, , % "ahk_id " (H := This.HWND)                                                	; LVM_GETEDITCONTROL
		...
		SciTEOutput(This.ItemText ","  This.Item ", " This.SubItem )
		  ...
		  VarSetCapacity(RECT, 16, 0), NumPut(This.SubItem, RECT, 4, "Int")
		  ...
		  If (This.SubItem = 0) {
			 ...
			 This.CW := This.EW := ErrorLevel
		  }
		  Else
			 This.CW := This.EW := NumGet(RECT, 8, "Int") - NumGet(RECT, 0, "Int")

		  ; Check the column alignement
		  VarSetCapacity(LVCOL, 56, 0), NumPut(1, LVCOL, "UInt") ; LVCF_FMT
	   ...
	   NM_DBLCLICK(L) {
		  ...
		  If (Item >= 0) && (SubItem >= 0) {
			 ...
			 If !(This.Next) {
				...
				This.ColCount := V - 1
				cell := This.GetCellSizes(H)  ; modified code
				...
				This.DW 	:= ErrorLevel
				SBW     	:= 0
				If (cell.Styles & 0x200000) ; WS_VSCROLL
				   SysGet, SBW, 2
				This.SW := SBW
				ControlGetPos, LX, LY, LW, , , % "ahk_id " H
				This.LX := LX
				This.LY := LY
				This.LR := LX + LW - (This.DX * 2) - This.SBW
				This.LW := LW
	   ...
	   GetCellSizes(HWND) {
			...
			This.DX 	:= NumGet(WINDOWINFO, 20, "Int")	- NumGet(WINDOWINFO, 4, "Int")  ; ClientX - WindowX
			This.DY  	:= NumGet(WINDOWINFO, 24, "Int")	- NumGet(WINDOWINFO, 8, "Int")
			cell.CW 	:= NumGet(WINDOWINFO, 28, "Int") 	- NumGet(WINDOWINFO, 20, "Int") ; width of this cell
			cell.Styles	:= NumGet(WINDOWINFO, 36, "UInt")
	   return cell
	   }
	   NextSubItem(K) {
		   ; -------------------------------------------------------------------------------------------------------------------
	   ...
	   RegisterHotkeys(Register = True) {
		  ...
		  If (Register) { ; Register
			 ...
			 OnMessage(0x312, This.HotkeyFunc) ; WM_HOTKEY
		  }
		  Else { ; Unregister
			 ...
			 OnMessage(0x312, This.HotkeyFunc, 0) ; WM_HOTKEY
			 This.HotkeyFunc := ""
		  }
	   }
