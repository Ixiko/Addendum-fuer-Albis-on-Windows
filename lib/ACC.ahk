...
;accDescription            	Read-only        	Note  The accDescription property is not supported in the transition to UI Automation. Microsoft Active Accessibility servers and applications should not use it.
...
;accNavigate                                     	Note  The accNavigate method is deprecated and should not be used. Clients should use other methods and properties such as AccessibleChildren, get_accChild, get_accParent, and IEnumVARIANT.
...
class ACC_FILTERS {
    byDefaultAction(oAcc,action){
        bf := oAcc.accDefaultAction = action
        return bf
    }

    byDescription(oAcc,desc){
        bf := oAcc.accDescription = desc
        return bf
    }

    byValue(oAcc,value){
        bf := oAcc.accValue = value
        return bf
    }

    byHelp(oAcc,help){
        bf := oAcc.accHelp = help
    }
    ...
    byRole(oAcc,role){
        bf := oAcc.accRole = role
        return bf
    }

    byName(oAcc,name){
        bf := oAcc.accName = name
        return bf
    }
...
Acc_Get(Cmd, ChildPath="", ChildID=0, WinTitle="", WinText="", ExcludeTitle="", ExcludeText="") {
    ...
    if ComObjType(AccObj, "Name") != "IAccessible"
        ...
        Loop Parse, ChildPath, ., %A_Space%
            try {
                if A_LoopField is digit
                    Children:=Acc_Children(AccObj), mg2:=A_LoopField ; mimic "m2" output in else-statement
                else
                    RegExMatch(A_LoopField, "(\D*)(\d*)", mg), Children:=Acc_ChildrenByRole(AccObj, mg1), mg2:=(mg2?mg2:1)
        ...
        try {
            if (Cmd = "Location")
                AccObj.accLocation(ComObj(0x4003,&xg:=0), ComObj(0x4003,&yg:=0), ComObj(0x4003,&wg:=0), ComObj(0x4003,&hg:=0), ChildId)
