...
; ..............: Aug. 	04, 2020      	- v2.2 	- 	Fixed 64bit neg. pointer VirtualAllocEx handle return
; ..............:                                                	https://www.autohotkey.com/boards/viewtopic.php?p=345509#p345509
;
; ----------------------------------------------------------------------------------------------------------------------
...
ExtListView_ToggleSelection(ByRef objLV, bSelect:=1, nItem:=-1) {	; Select/deselect items in the target ListView.
    ...
    NumPut( 0x0008,                              	LVITEM, 0, 	"UInt"  )  	; mask      	= LVIF_STATE    	= 0x0008.
    NumPut( nItem,                                 	LVITEM, 4, 	"Int"  )    	; iItem.
    NumPut( 0,                                       	LVITEM, 8, 	"Int"  )    	; iSubItem.
    NumPut( (bSelect) ? state : 0x0000,    	LVITEM, 12, 	"UInt"  ) 	; state       	= 0x0000 (reset mask).
    ...
    If ( !DllCall( "WriteProcessMemory", Ptr,objLV.hproc, Ptr,objLV.pwritebuf, Ptr,&LVITEM, "UInt",objLV.szwritebuf, "UInt",0 ) ) {
		...
		SciTEOutput("objLV.pwritebuf: error writing memory" ", WriteProcessMemory`nLastError: " A_LastError "`n")
		return 0
...
ExtListView_Select(ByRef objLV, row, select:=true) {
	...
	If select {
		...
		nsmask	:= LVIS_FOCUSED | LVIS_SELECTED
	} else {
		nstate	:= 0x0
		nsmask	:= 0x0
	}
	...
	VarSetCapacity(  LVITEM  	, 46	, 0)

	NumPut(nmask 	, LVITEM	, 0	,	"uint") 	; mask
	NumPut(row     , LVITEM	, 4	,	"int") 		; iItem
	NumPut(nCol	, LVITEM	, 8	,	"uint") 	; subitem
	NumPut(nstate	, LVITEM	, 12	,	"uint") 	; state
	NumPut(nsMask, LVITEM	, 16	,	"uint") 	; stateMask

	;DllCall("SendMessage", "uint", objLV.hLV, "uint", 4103, "uint", 0, "uint", &lvItem)
	SendMessage, % LVM_SETITEMSTATE, % row, &LVITEM,, % "ahk_id " objLV.hLV
...
ExtListView_GetItemText(ByRef objLV, nRow, nCol) {                       	; Get the text of the desired item
    ...
    If ( !DllCall( "ReadProcessMemory", Ptr,objLV.hproc, Ptr,objLV.preadbuf, Ptr,&cRecvBuf, "UInt",objLV.szreadbuf, Ptr,0 ) )
...
ExtListView_Initialize(sWnd, szReadBuf:=1024, sEnc:="CP0") {       	; Initialize the object containing ListView's related data
	...
	objLV.dwProcessId := dwProcessId

    ; PROCESS_VM_OPERATION = 0x0008
	; PROCESS_VM_READ        	= 0x0010
	; PROCESS_VM_WRITE      	= 0x0020
    If !(objLV.hproc := DllCall( "OpenProcess", "UInt",0x0008|0x0010|0x0020, "Int",0, "UInt",dwProcessId, "Ptr" ))
...
__ExtListView_AllocateMemory(ByRef objLV) {                                	; Allocates memory into the target process
	...
	If ( !(objLV.pwritebuf	:= DllCall( "VirtualAllocEx", Ptr,objLV.hproc, Ptr,0, UInt,objLV.szwritebuf, UInt,0x1000 , UInt,0x4, "UPtr" )) ) ; <<<<<<<<<<
        Return 0
    If ( !(objLV.preadbuf 	:= DllCall( "VirtualAllocEx", Ptr,objLV.hproc, Ptr,0, UInt,objLV.szreadbuf, UInt,0x1000 , UInt,0x4, "UPtr" )) ) ; <<<<<<<<<<
        Return 0
